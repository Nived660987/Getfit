...
import StyledProgressbar from '../Meditation/StyledProgressbar';
import Sound from 'react-sound';
import SoundComponent from '../Meditation/playSound';
import 'react-circular-progressbar/dist/styles.css';
import '../Meditation/Meditation.css';
...
class Meditation extends Component {
	...
	volumeChange = (event) => {
		...
		this.setState({
			...
			volumeIcon:
				this.state.mute || newVolume === 0 ? noVolumeIcon : newVolume <= 50 ? quietVolumeIcon : loudVolumeIcon,
		});
	};
	...
	render() {
		...
		const timeOptions = this.state.timeValues.map((duration) => (
			<button
				...
				className={!this.state.timeHovered && duration === this.state.desiredTime ? 'active' : ''}
				onClick={() => {
		...
		const audioOptions = this.state.audioNames.map((audioName) => (
			<button
				...
				className={
					!this.state.audioHovered && this.state.audioUrl === 'audio/' + audioName.toLowerCase() + '.mp3'
						? 'active'
						: ''
				}
				onClick={() => {
